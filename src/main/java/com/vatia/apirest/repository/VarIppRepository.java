...
public interface VarIppRepository extends JpaRepository<VarIpp, Integer> {
	...
	@Query(value = "SELECT * FROM VARIPP I WHERE I.ANIO = ?1 ORDER BY ANIO ASC", nativeQuery = true)
	VarIpp getVarIppAnio(Integer anio);  
	...
	@Query(value = 
			...
			"FROM IPP WHERE ANIO = ?2) V"
			, nativeQuery = true)
	JSONObject getVariacionIppAnio(Integer anioAnterior,Integer anioActual);
}
