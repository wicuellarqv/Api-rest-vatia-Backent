...
public interface ConsultaRepository extends JpaRepository<Contratos, Integer>{
	...
	final String QUERY_DETAIL_CONTRACT = "Select distinct \r\n"
			...
			+ "			 ACC.str_codigo_sic_agente_ccial as codigo_sic_comprador,\r\n"
			+ "			 ACC.str_nombre_agente_ccial as comprador,  \r\n"
			+ "			 ACV.str_codigo_sic_agente_ccial as codigo_sic_vendedor,\r\n"
			...
			+ "			 	left join tbl_negoc_contratos as NC ON NC.num_id_negoc_contrato = C.num_id_negoc_contrato  \r\n"
			+ "			 	left join tbl_agentes_comerciales as ACC ON ACC.num_id_agente_ccial = C.num_id_agnte_ccial_comp  \r\n"
			+ "			 	left join tbl_agentes_comerciales as ACV ON ACV.num_id_agente_ccial = C.num_id_agnte_ccial_vend  \r\n"
			+ "			 	left join tbl_condiciones_tipos_contratos as CTC ON  CTC.num_id_condicion_tipo_contrato = C.num_id_condicion_tipo_contrato  \r\n"
			+ "			 	left join tbl_tipos_mercados as TM ON TM.num_id_tipo_mercado = C.num_id_tipo_mercado  \r\n"
			+ "			 	left join tbl_tipos_contratos tc on c.num_id_tipo_contrato=tc.num_id_tipo_contrato \r\n"
			+ "			 	left join tbl_precios_contratos pc on c.num_id_contrato=pc.num_id_contrato \r\n"
			+ "			 	left join tbl_cantidad_contratos cc on c.num_id_contrato=cc.num_id_contrato \r\n"
			+ "			 	left join tbl_garantias_contratos gc on c.num_id_contrato=gc.num_id_contrato \r\n"
			+ "			 	left join tbl_tipos_precio tp on tp.num_id_tipo_precio=pc.num_id_tipo_precio \r\n"
			+ "			 	left join tbl_formulas_precios fp on fp.num_id_formula_precio=pc.num_id_formula_precio \r\n"
			+ "			 	left join tbl_tipos_cantidad tca on cc.num_id_tipo_cantidad=tca.num_id_tipo_cantidad \r\n"
			+ "			 	left join tbl_tipos_garantias tga on tga.num_id_tipo_garantia = gc.num_id_tipo_garantia \r\n"
