...
public interface ClienteRepository extends PagingAndSortingRepository<Cliente, ClientePk> {
	...
	@Query(value = "SELECT DISTINCT CATEGORIA_CLIENTE  "
			+ "			  FROM STG_TRADEBALANCE.dbo.STG_DIM_CLIENTE WHERE "
			+ "CATEGORIA_CLIENTE IS NOT NULL AND FECHA_ESTADO_HIS_MER IS NOT NULL", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM STG_TRADEBALANCE.dbo.STG_DIM_CLIENTE   "
			+ "			  where NOMBRE_RAZON_SOCIAL like :razon AND CATEGORIA_CLIENTE IS NOT NULL AND FECHA_ESTADO_HIS_MER IS NOT NULL "
	...
	@Query(value = "SELECT * FROM STG_TRADEBALANCE.dbo.STG_DIM_CLIENTE   "
			+ "			  where CATEGORIA_CLIENTE = :categoria AND FECHA_ESTADO_HIS_MER IS NOT NULL "
			+ "			  order by NOMBRE_FACTURACION ASC", nativeQuery = true)
	public Page<Cliente> nativeFindByLikeClienteCatPage(Pageable page, @Param("categoria") String categoria);

	@Query(value = "SELECT * FROM STG_TRADEBALANCE.dbo.STG_DIM_CLIENTE   "
			+ "			  where CATEGORIA_CLIENTE = :categoria and   "
			+ "			  NOMBRE_RAZON_SOCIAL like :razon  AND CATEGORIA_CLIENTE IS NOT NULL  AND FECHA_ESTADO_HIS_MER IS NOT NULL "
			+ "			  order by NOMBRE_FACTURACION ASC", nativeQuery = true)
